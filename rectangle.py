...
featclass = r"H:\ArcGIS_Training\Shp\myfgdb.gdb\population1"

with arcpy.da.SearchCursor(featclass,['LATITUDE','LONGITUDE','HEIGHT','WIDTH']) as cursor:
    print(cursor)
    for coord in cursor: # Iterating through the list created above
        print(coord)
        # print("State: {0}, County: {1}, Population: {2}".format(
        #     coord.getValue("STATE_NAME"),
        #     coord.getValue("NAME"),
        #     coord.getValue("POP2000")))

        latitude = coord[0]
        longitude = coord[1]
        height = coord[2]
        width = coord[3] # Assigning the values from the list above
...
arcpy.CopyFeatures_management("in_memory\\Polygons",r"H:\ArcGIS_Training\Shp\myfgdb.gdb\population2")